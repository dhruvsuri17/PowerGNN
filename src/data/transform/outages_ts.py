...
def transform_outages_ts(
    ...
    to_add = pd.DataFrame(
        data={
            "gen_name": missing_gens,
            "datetime": [datetime(2024, 1, 1, 0, 0, 0)] * len(missing_gens),
            "in_service": [True] * len(missing_gens),
            "in_outage": [False] * len(missing_gens),  # Add this column to match `outages`
        }
    )

    to_add = to_add[outages.columns.intersection(to_add.columns)]
